...
public class Case06 {
	...
	public void testCase6_1() throws InterruptedException {
		...
		String password2 = "StudentAA01A";

		// モック対象メソッドの返却値を設定
		when(loginService.getLoginInfo(userId, password2)).thenReturn("");
		when(loginUserUtil.isStudent()).thenReturn(true);
		when(loginUserUtil.sendDisp()).thenReturn("redirect:/course/detail");

		getRequest = MockMvcRequestBuilders.post("/login").param("loginId", userId).param("password", password2);
